 ...
 $the_id= $_GET['delete'];
 $query="DELETE FROM personne WHERE id= $the_id";
 $result=mysqli_query($connect,$query);
  if ($result) {
    header('Location:liste_personnes.php');
...
<div class="col-xs-4">
  ...
  <a class="btn btn-success"href="data_personne.php?source=add_personne">Ajouter une personne</a>
</div>
...
if (isset($_POST['checkboxArray'])) {
  foreach ($_POST['checkboxArray'] as $checkPostId_Value) {
    ...
    case 'editer':
        ...
        break;
    case 'Supprimer':
    $query_delete="DELETE FROM personne  WHERE id=$checkPostId_Value";
    $result_delete=mysqli_query($connect,$query_delete);
    if (!$result_delete) {
      die('Erreur de supression'.mysqli_error($connect));
    }
    header('Location:liste_personnes.php?message=Suppression faite!');
    break;
